...
define(function (require) {
    ...
    /**
     * Tip的触发器的默认配置，触发器规定了触发的class以及其它选项
     *
     * @public
     * @const
     * @type {Object}
    ...
    Tip.prototype = {
         ...
         */
        binds: [
                'onResize', 'onDocClick', 'onShow', 'onHide',
                'hide', 'onMouseenter', 'onMouseleave'
               ].join(','),
         ...
         *
         * @public
         * @param {(Object | Array.<Object>)} triggers
         */
        addTriggers: function (triggers) {
            var me = this;
            var events = this.events;
            var extend = T.object.extend;

            var triggers = T.lang.isArray(triggers) ? triggers : [triggers];
            T.each(triggers, function (trigger) {
                trigger = extend(
                    extend({}, Tip.DEFAULT_TRIGGER_OPT),
                    trigger || {}
                );

                if (events[trigger.mode]) {

                    trigger.clazz = T.lang.isArray(trigger.clazz)
                        ? trigger.clazz
                        : [trigger.clazz];
        ...
        getTrigger: function (target) {
            for (var i = 0; i < this.triggersClazz.length; i++) {
                var clazz = this.triggersClazz[i];
                if (T.dom.hasClass(target, clazz)) {
                    return this.triggers[clazz];
                }
            }
        },
        ...
        onMouseleave: function() {
            var trigger = this.curTrigger;
            this.clear();
            this.timer = setTimeout(this.hide, trigger.hideDelay);
        },
        ...
        onShow: function (e) {
            ...
            var trigger = this.getTrigger(target);
            ...
            if (events) {
                ...
                else if (trigger.mode === 'over') {
                    T.on(this.main, 'mouseenter', this.onMouseenter);
                    T.on(this.main, 'mouseleave', this.onMouseleave);
                }
            }
        ...
        hide: function () {
            ...
            if (events && target) {
                ...
                if (trigger.mode === 'click') {
                    ...
                    T.un(main, 'mouseenter', this.onMouseenter);
                    T.un(main, 'mouseleave', this.onMouseleave);
                }
            }
